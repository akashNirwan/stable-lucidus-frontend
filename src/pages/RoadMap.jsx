...
const RoadMap = () => {
  ...
  const { roadmap, roadmapLoading, error } = useSelector(
    ...
    <div className="text-white">
      <div className="bg-[#0f0630] min-h-screen text-white p-4">
        <div className="flex items-center gap-2 mb-6">
          ...
          <h1 className="text-[20px] font-semibold">{career}</h1>
        </div>
        ...
        <div className="bg-[#261172] rounded-2xl px-4 flex items-center gap-4 w-fit">
          ...
          <p className="text-base font-medium max-w-xs">
            Kickstart your career exploration in Grade{" "}
            {roadmap?.[0]?.career?.[0]?.grades?.[0]?.grade} !
          </p>
        </div>
