...
export default function CareerSurvey() {
  ...
  const { saveFeedbackLoading } = useSelector((state) => state.microexperience);
  ...
  const handleNext = () => {
    ...
    dispatch(saveBadge(payload)).then((res) => {
      if (
        (res.payload && res.payload.code === 201) ||
        res.payload.statusCode === 200
      ) {
        navigate(
          `/student-choice?questionId=${questionId}&careerId=${careerId}`
        );
      }
    });
  };
